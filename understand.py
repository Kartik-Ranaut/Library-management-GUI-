            ...
            def submit_book():
                  ...
                  if (userid=="" or bookid=="" ):
                        ...
                        try:
                              userid=int(userid)
                              bookid=int(bookid)
                              sql=ms.connect(host='localhost',user='root',passwd='2008',database='project')
                              cur=sql.cursor()
                              cur.execute("select * from publish_data where user_id=%s and book_id=%s ",(userid,bookid))
                              data=cur.fetchall()
                              count=cur.rowcount
                              cur.execute("select date from publish_data where user_id=%s and book_id=%s ",(userid,bookid))
...
Dear Student you have submitted the book to our library today.\n date of issue:"""+str(datesql)+"date of submission:"+str(date1)+"\nDetails \nUSER ID:" +str(userid)+'\nBOOK ID '+str(bookid)+"\n Hope you enjoyed the reading :) ")
                                    ...
                                    server.sendmail("your mail",emailid,message)
                                    server.quit()
                              send_mail()
...
Dear Student you have published a book from our library today. \nWe urge you to submitted it before 7 day otherwise extra charges will be applied. \n Details \nUSER ID:""" +str(userid)+'\nBOOK ID '+str(bookid)+'\nEMAIL ID '+str(emailid)+'\nDate: '+str(date1)+"""\n Hope you will enjoy the book :) \n Happy reading.""")
                              server=SMTP_SSL("smtp.gmail.com", 465)
                              server.login("enter your mail_id", "enter your password")
                              server.sendmail("your mailid",emailid,message)
                              server.quit()
                        if (userid=="" or bookid=="" or emailid==""):
                              mb.showinfo("Error","all fields are required",parent=main3)
                        else:
                              send_mail()
                              sql=ms.connect(host='localhost',user='root',passwd='rsharma@123',database='project')
                              cur=sql.cursor()
...
#           # my sql connectivity......................................
            ...
            tb=ttk.Treeview(frame,columns=(1,2,3,4),show="headings",height=h)
            tb.pack()
...
def login_user():
      ...
      if user_info=="" or pass_info=="" :
            ...
            if (user_info=="kartik" and pass_info=="2008") :
                count=1
            else:
                count=0
            if count==0:
